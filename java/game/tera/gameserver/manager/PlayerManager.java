...
import tera.gameserver.network.serverpackets.S_Pet_Incubator_Info_Change;
import tera.gameserver.network.serverpackets.FriendListInfo;
import tera.gameserver.network.serverpackets.HotKey_S_Load_Client_Account_Setting;
...
import tera.gameserver.network.serverpackets.S_Login;
import tera.gameserver.network.serverpackets.S_Select_User;
import tera.gameserver.network.serverpackets.S_Artisan_Recipe_List;
import tera.gameserver.network.serverpackets.S_Available_Event_Matching_List;
import tera.gameserver.network.serverpackets.S_Available_Social_List;
import tera.gameserver.network.serverpackets.S_Brocast_Guild_Flag;
import tera.gameserver.network.serverpackets.S_Current_Election_State;
import tera.gameserver.network.serverpackets.S_Daily_Quest_Complete_Count;
import tera.gameserver.network.serverpackets.S_Festival_List;
import tera.gameserver.network.serverpackets.S_Ftp_PremiumUser_Permission;
import tera.gameserver.network.serverpackets.S_Masstige_Status;
import tera.gameserver.network.serverpackets.S_Npcguild_List;
import tera.gameserver.network.serverpackets.S_Ongoing_Hunting_Event_List;
import tera.gameserver.network.serverpackets.S_Pcbanginventory_Datalist;
import tera.gameserver.network.serverpackets.Settings;
import tera.gameserver.network.serverpackets.S_Skill_List;
import tera.gameserver.network.serverpackets.S_Spawn_Me;
import tera.gameserver.network.serverpackets.S_Artisan_List;
import tera.gameserver.network.serverpackets.S_Clear_Quest_Info;
import tera.gameserver.network.serverpackets.S_Completed_misson_Info;
import tera.gameserver.network.serverpackets.S_Load_Hint;
import tera.gameserver.network.serverpackets.S_Account_Benefit_List;
import tera.gameserver.network.serverpackets.S_Pet_Info_Clear;
import tera.gameserver.network.serverpackets.S_Send_User_Play_Time;
import tera.gameserver.network.serverpackets.S_User_External_Change;
import tera.gameserver.network.serverpackets.S_Virtual_Latency;
import tera.gameserver.network.serverpackets.S_Move_Distance_Delta;
import tera.gameserver.network.serverpackets.S_My_Description;
import tera.gameserver.network.serverpackets.S_Notify_Change_Class_And_Elite;
import tera.gameserver.network.serverpackets.S_Player_Stat_Update;
...
public final class PlayerManager
{
	 ...
	 */
	public synchronized void createPlayer(UserClient client, Playerdetails2 detailsappearance,PlayerAppearance appearance, String name, PlayerClass playerClass, Race race, Sex sex)
	{
		...
		// устанавливаем внешнсоть
		//player.setdetails2(appearance, false);
		...
		// Update New Details character_presets_2
		dbManager.insertPlayerPresets2(detailsappearance);
	...
	public void selectPlayer(UserClient client, int objectId)
	{
		...
		player.sendPacket(S_Brocast_Guild_Flag.getInstance(), false);
		player.sendPacket(S_Current_Election_State.getInstance(), false);
		player.sendPacket(S_Ongoing_Hunting_Event_List.getInstance(), false);
		...
		player.sendPacket(S_Artisan_List.getInstance(), false);
		...
		player.sendPacket(S_Ftp_PremiumUser_Permission.getInstance(), false);
		player.sendPacket(S_Notify_Change_Class_And_Elite.getInstance(), false);
		player.sendPacket(S_Masstige_Status.getInstance(), false);
		player.sendPacket(S_Available_Event_Matching_List.getInstance(), false);
		//player.sendPacket(S_User_External_Change.getInstance(player), true);  // needs porting over
		//player.sendPacket(S_User_Item_Equip_Changer.getInstance(player), true); // needs porting over
		...
		player.sendPacket(S_Send_User_Play_Time.getInstance(), false);
		player.sendPacket(S_Pcbanginventory_Datalist.getInstance(), false);
		player.sendPacket(S_Completed_misson_Info.getInstance(), false);
		player.sendPacket(S_Daily_Quest_Complete_Count.getInstance(), false);
		player.sendPacket(S_Available_Event_Matching_List.getInstance(), false);
