...
public class NpcInfo extends ServerPacket
{
	...
	protected void writeImpl(ByteBuffer buffer)
	{
		...
		int shiftreplace = buffer.position();
		...
		writeInt(buffer, 0);// new 00000000
		writeByte(buffer, 0);// new 00
		writeInt(buffer, 0xB9ACC5D8);// new D8C5ACB9
		writeInt(buffer, 0x0000CE74);// new 74CE0000
		int shift = buffer.position();
		buffer.position(shiftreplace);
		writeShort(buffer, shift - 8);// write shift
